...
with DAG('covid_ddl_counties', schedule_interval='@once', default_args=default_args, catchup=False, template_searchpath='/opt/airflow/') as dag:

    extracting = DummyOperator(task_id='extracting')

    with TaskGroup('creating_county_ddl') as processing_group:
    ...
    createFunction = PostgresOperator(task_id='create_functions',postgres_conn_id='postgres_new',sql="sql/create_trigger.sql",database="userdata")
...
extracting >> processing_group >> loading >> createFunction >> getdata >> loadtodb
