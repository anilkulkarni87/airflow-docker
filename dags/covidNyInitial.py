...
with DAG('NY_COVID_INITIAL_LD', schedule_interval='@once', default_args=default_args, catchup=False, template_searchpath='/opt/airflow/') as dag:
    ...
    with TaskGroup('INIT_DB_DDL') as createCountyTables:
        for county in nycounties:
            ...
            county = county.replace(".", "")
            PostgresOperator(task_id='init_covid_table_'+str(totalcounties), postgres_conn_id='postgres_new',
                             sql="sql/covid_init.sql", params={'countyname': county}, database="userdata")

    createFunction = PostgresOperator(
        task_id='CREATE_FUNCTION', postgres_conn_id='postgres_new', sql="sql/create_trigger.sql", database="userdata")

    createMasterTable = PostgresOperator(
        task_id='CREATE_MASTER', postgres_conn_id='postgres_new', sql="sql/create_master.sql", database="userdata")
